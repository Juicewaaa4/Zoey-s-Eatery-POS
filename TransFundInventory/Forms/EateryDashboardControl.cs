...
{
    public class EateryDashboardControl : UserControl
    {
        ...
        private Label lblSalesBreakdown = null!;
        ...
        private void InitializeComponent()
        {
            ...
            var card1 = CreateStatCard("🍽️ Menu Items", "0", Color.FromArgb(230, 126, 34));
            lblTotalProducts = (Label)card1.Controls[0];
            var card2 = CreateStatCard("💵 Avg Menu Price", "₱0.00", Color.FromArgb(56, 142, 60));
            lblTotalValue = (Label)card2.Controls[0];
            var card3 = CreateStatCard("🚨 LOW STOCK (Drinks)", "0", Color.FromArgb(211, 47, 47));
            lblLowStock = (Label)card3.Controls[0];
            var card4 = CreateStatCard("📅 Auto-Daily Sales", "₱0.00", Color.FromArgb(41, 128, 185));
            card4.Height = 110;
            ...
            {
                Text = "📋 Recent Eatery Transactions",
                Font = new Font("Segoe UI", 11, FontStyle.Bold),
                ForeColor = Color.FromArgb(27, 94, 32),
                Dock = DockStyle.Top,
                Height = 30,
        ...
        private void LoadDashboardData(DateTime? filterDate = null)
        {
            try
            {
                ...
                if (products.Count > 0)
                {
                    lblTotalValue.Text = $"₱{products.Average(p => (double)p.Price):N2}";
                }
                else
                {
            ...
            {
                MessageBox.Show($"Error loading eatery dashboard: {ex.Message}", "Error",
        ...
        private void ExportDashboardSales()
        {
            ...
            var path = ExportHelper.ShowSaveDialog("Excel Files|*.xlsx", $"EaterySales_{targetDate:yyyyMMdd}.xlsx");
            if (path != null)
            {
                ...
                ExportHelper.ExportToExcel(exportList, "Daily Eatery Sales", path);
                MessageBox.Show("Eatery Sales Exported Successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
